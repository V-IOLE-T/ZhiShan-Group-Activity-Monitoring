...
系统不再使用秒级 Pin 轮询。当前采用每周定时审计：

1. 每周一 `09:00` 执行一次任务。
2. 扫描群内当前 Pin 列表。
3. 仅处理“上周新增且未处理”的 Pin 消息（按 Pin 操作时间判定，周一到周日）。
4. 对每条命中消息执行：
   - 归档 Pin 记录（配置了 `PIN_TABLE_ID` 时）
   - 给发送者增加“被Pin次数”
   - 可选写入精华文档（配置了 `ESSENCE_DOC_TOKEN` 时）
5. 若上周有新增 Pin，群里发送 1 张“上周加精”汇总卡片；若无新增则不发卡。

## 执行入口与调度

- 启动入口：`python main.py`
- 调度注册：`pin_scheduler.py`（每周一 `09:00`）
