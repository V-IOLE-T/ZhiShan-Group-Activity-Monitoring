...
class PinMonitor:
    ...
    def get_pinned_messages(self):
        ...
        params = {"chat_id": self.chat_id}  # 修正: 使用chat_id而不是container_id
    ...
    def send_pin_notification(self, message_id, pin_info):
        ...
        left_column_elements = [
            {"tag": "markdown", "content": f"**Pin操作人**\n{operator_name}"},
            {"tag": "markdown", "content": f"**话题发起人**\n{sender_name}"},
        ]

        # 构建右侧元素(话题内容)
        right_column_elements = [{"tag": "markdown", "content": f"**话题内容**\n{content}"}]
        # 不显示附件
        ...
        card_content = {
            ...
            "header": {
                "template": "orange",
                "title": {"tag": "plain_text", "content": "🌟 新增加精话题"},
            },
            "elements": [
                {
                    "tag": "column_set",
    ...
    def _handle_new_pin(self, message_id, pins):
        ...
        if image_keys:
            for img_key in image_keys:
                ...
                file_token = self._download_and_upload_resource(
                    message_id, img_key, "image", f"{img_key}.png"
                )
                if file_token:
        ...
        msg_create_time_str = datetime.fromtimestamp(int(msg_create_time) / 1000).strftime(
            "%Y-%m-%d %H:%M:%S"
        )
    ...
    def _download_and_upload_resource(self, message_id, file_key, resource_type, file_name):
        ...
        headers = {"Authorization": f"Bearer {self.auth.get_tenant_access_token()}"}
    ...
    def _upload_to_drive(self, file_content, file_name):
        ...
        upload_headers = {"Authorization": f"Bearer {self.auth.get_tenant_access_token()}"}
